...
public struct DocumentInfo {
    ...
    private static var tesseract: G8Tesseract = {
        ...
        NSLog("Train data path: \(trainDataPath)")
        
        let cacheURL = NSFileManager.defaultManager().URLsForDirectory(.CachesDirectory, inDomains: .UserDomainMask).first!

        let tessdataURL = cacheURL.URLByAppendingPathComponent("tessdata", isDirectory: true)!
        ...
        let tesseract = G8Tesseract(language: "eng", configDictionary: [:], configFileNames: [], cachesRelatedDataPath: cacheURL.path!, engineMode: .TesseractOnly)
